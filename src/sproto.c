...
int 
sproto_encode(struct sproto_type *st, void * buffer, int size, 
										sproto_callback cb, void *ud) {
	uint8_t * header = buffer;
	uint8_t * data;  
	...
	if (size < header_sz)
		return -2;
	data = header + header_sz;//printf("===>header_sz:%d, st->maxn:%d\n", header_sz, st->maxn);
	size -= header_sz;
	int i;
	int index = 0;
	int lasttag = -1;
	...
	}//printf("=====%d, %d, %d=====\n", index, datasz, SIZEOF_HEADER + index * SIZEOF_FIELD + datasz);
